...
## 4. a file geodatabse for clipped outputs - can be named anything.

setwd("your file path here")

##Obtain top level file path
fp <- getwd()
fp <- gsub("/", "\\", fp, fixed = TRUE) # Need 2 backslashes to create 1. One is an escape character.
...
## Write out txt files for Python script parameters. Note no quotes and when Python imports
## the text files it doubles the back slashes.
write.table(df$input, "input.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(df$clip, "UMclip.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(df$output, "output.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
